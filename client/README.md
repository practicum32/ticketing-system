...
- **Solution:** Install Node.js from https://nodejs.org or refresh environment variables:
```powershell
$env:Path = [System.Environment]::GetEnvironmentVariable("Path","Machine") + ";" + [System.Environment]::GetEnvironmentVariable("Path","User")
```

**Issue: "Port 5199 already in use"**
...
- **Solution:** Make sure server runs on port 5199 and client on port 5173. Program.cs already contains proper CORS configuration.

**Issue: Data disappears**
- **Explanation:** This is normal - data is in memory and deleted when server stops. It's not a bug!

### 🌐 Environment Variables / משתני סביבה

**עברית:**

הפרויקט משתמש בקובץ `.env` בתיקיית client:

```
VITE_API_BASE_URL=http://localhost:5199
```

ניתן לשנות את כתובת השרת אם הוא רץ על פורט או מכונה אחרת.

**English:**

The project uses `.env` file in the client directory:

```
VITE_API_BASE_URL=http://localhost:5199
```

You can change the server address if it runs on a different port or machine.

### 👨‍💻 Development

Built as a home assignment for ticketing system management.
